...
class EmbeddingGenerator:
    ...
    def _setup_cache_directory(self) -> Path:
        ...
        # Get cache folder from config structure
        cache_folder = (
            self.config.get("local", {})
            .get("settings", {})
            .get("cache_folder", "./cache/embeddings/")
        )
    ...
    @glass_box_boundary(side_effect_check=False)  # Pure function
    def _load_model_info(self) -> Dict[str, EmbeddingModelInfo]:
        ...
        if "embedding_models" in self.config:
            config_data = self.config["embedding_models"]
        else:
            config_data = self.config

        # Local models
        local_models = config_data.get("local", {}).get("models", {})
        ...
        # Cloud models (OpenAI)
        openai_config = config_data.get("cloud", {}).get("openai", {})
        if openai_config:
            model_info["openai"] = EmbeddingModelInfo(
                name=openai_config.get("model", "text-embedding-ada-002"),
                provider="openai",
        ...
        # Cloud models (Cohere)
        cohere_config = config_data.get("cloud", {}).get("cohere", {})
        if cohere_config:
            model_info["cohere"] = EmbeddingModelInfo(
                name=cohere_config.get("model", "embed-english-v3.0"),
                provider="cohere",
    ...
    def _get_cached_embedding(
        ...
        cache_enabled = (
            self.config.get("local", {}).get("settings", {}).get("cache_folder")
            is not None
        )
    ...
    def generate_embeddings(
        ...
        if model == "local":
            model = self.config.get("local", {}).get("default", "all-MiniLM-L6-v2")

        # Use defaults if not specified
        if batch_size is None:
            batch_size = (
                self.config.get("local", {}).get("settings", {}).get("batch_size", 32)
            )
        if normalize is None:
            normalize = (
                self.config.get("local", {})
                .get("settings", {})
                .get("normalize_embeddings", True)
            )
