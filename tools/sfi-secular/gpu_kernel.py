...
by verify.py using the DVF (Deterministic Verified Fallback) pattern.
...
Standard: Yeshua
testable_failure_condition: GPU output hash differs from CPU reference hash
testable_failure_condition: GPU path executes when CUDA is unavailable
"""
...
def gpu_compute_optical_flow(
    ...
    testable_failure_condition: output differs from CPU reference for same inputs
    testable_failure_condition: returns non-None when CUDA is unavailable
    """
...
# ---------------------------------------------------------------------------
# DVF (Deterministic Verified Fallback) Pipeline
# ---------------------------------------------------------------------------

def self-contained_interpolate(
    frame_a: np.ndarray,
    frame_b: np.ndarray,
) -> np.ndarray:
    """
    Self-Contained frame interpolation with automatic GPU/CPU fallback.

    Attempts GPU path first. If GPU unavailable or output doesn't
    match CPU reference, falls back to CPU path silently (DVF (Deterministic Verified Fallback)).
    ...
    testable_failure_condition: returns frame identical to frame_a or frame_b
    """
    # Try GPU path
...
if __name__ == "__main__":
    ...
    result = self-contained_interpolate(frame_a, frame_b)
    print(f"Output shape: {result.shape}")
    print(f"Output hash: {frame_sha256(result)[:16]}")

    result2 = self-contained_interpolate(frame_a, frame_b)
