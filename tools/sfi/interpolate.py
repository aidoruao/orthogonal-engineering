...
Standard: Yeshua
falsifies_if: output frame hash differs from expected for given inputs
"""
...
def compute_optical_flow(
    ...
    falsifies_if: flow shape != (H, W, 2) for valid inputs
    """
    try:
        import cv2
    except ImportError:
...
def warp_frame(
    ...
    falsifies_if: output shape != input shape
    """
    H, W = flow.shape[:2]
...
def interpolate_frame(
    ...
    falsifies_if: output frame shape != input frame shape
    falsifies_if: output frame is all zeros or all ones
    falsifies_if: output is identical to either input frame (no interpolation occurred)
    """
...
def frame_sha256(frame: np.ndarray) -> str:
    ...
    falsifies_if: hash differs for identical inputs
    """
