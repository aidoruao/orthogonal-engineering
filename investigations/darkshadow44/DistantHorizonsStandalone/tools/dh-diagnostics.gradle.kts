...
tasks.register<DefaultTask>("dhDiagnostics") {
    ...
    doLast {
        ...
        // Phase 4: Multi-setting interaction analysis
        printInteractionAnalysis(findings)
        
        // Phase 4: Player scaling table
        printPlayerScalingTable(findings)
        
        // Phase 4: Recommended config generator
        generateRecommendedConfig(findings)
    }
}
...
/**
 * Phase 4: Multi-setting interaction analysis.
 * Checks combinations of settings that have compounding effects.
 */
fun printInteractionAnalysis(findings: List<ConfigFinding>) {
    println()
    println("=".repeat(80))
    println("SETTING INTERACTION ANALYSIS")
    println("=".repeat(80))
    println()
    
    val maxGenFinding = findings.find { it.setting == "maxGenerationRequestDistance" }
    val maxChunkRadiusFinding = findings.find { it.setting == "generationMaxChunkRadius" }
    
    if (maxGenFinding != null && maxChunkRadiusFinding != null) {
        val distance = maxGenFinding.currentValue
        val radius = maxChunkRadiusFinding.currentValue
        
        println("Configuration combination analysis:")
        println()
        println("  maxGenerationRequestDistance = $distance")
        println("  generationMaxChunkRadius = $radius")
        println()
        
        if (distance >= 4096 && radius == 0) {
            println("  [WARNING] High distance + unbounded radius creates compound load.")
            println("            Distance creates large area; unbounded radius removes limit.")
            println("            Recommendation: Set generationMaxChunkRadius to 128.")
        } else if (distance >= 2048 && radius == 0) {
            println("  [NOTICE] Moderate distance + unbounded radius.")
            println("           Consider setting explicit chunk radius limit.")
        } else if (distance >= 4096 && radius > 0) {
            println("  [NOTICE] High distance but bounded by radius = $radius.")
            println("           Chunk radius limit partially contains the load.")
        } else {
            println("  [OK] Settings within acceptable interaction range.")
        }
    } else {
        println("Insufficient data for interaction analysis.")
    }
    println()
}
...
/**
 * Phase 4: Player scaling table.
 * Shows expected load at various player counts.
 */
fun printPlayerScalingTable(findings: List<ConfigFinding>) {
    println()
    println("=".repeat(80))
    println("PLAYER SCALING TABLE")
    println("=".repeat(80))
    println()
    println("Expected generation load at various player counts:")
    println()
    
    val maxGenFinding = findings.find { it.setting == "maxGenerationRequestDistance" }
    val playerCounts = listOf(1, 2, 5, 10, 20, 50)
    
    if (maxGenFinding != null) {
        val distance = maxGenFinding.currentValue
        val areaPerPlayer = maxGenFinding.areaBlocksSquared
        
        println("With maxGenerationRequestDistance = $distance:")
        println()
        println("  %-8s %-20s %-25s".format("Players", "Total Area (blocks²)", "Est. Events"))
        println("  " + "-".repeat(55))
        
        for (players in playerCounts) {
            val totalArea = areaPerPlayer * players
            val estimatedEvents = (totalArea / 1000).toInt()
            val status = when {
                estimatedEvents > 50000 -> "[CRITICAL]"
                estimatedEvents > 20000 -> "[WARNING] "
                estimatedEvents > 5000 -> "[NOTICE]  "
                else -> "[OK]      "
            }
            println("  %-8d %-20s %-25s %s".format(
                players, "%,.0f".format(totalArea), estimatedEvents, status))
        }
        println()
        println("Est. Events assumes 1 event per 1000 blocks² (simplified model).")
    } else {
        println("maxGenerationRequestDistance not found in config analysis.")
    }
    println()
}
...
/**
 * Phase 4: Recommended config generator.
 * Outputs a config snippet that can be copy-pasted.
 */
fun generateRecommendedConfig(findings: List<ConfigFinding>) {
    println()
    println("=".repeat(80))
    println("RECOMMENDED CONFIGURATION")
    println("=".repeat(80))
    println()
    println("Copy and paste the following into your DH config:")
    println()
    println("-".repeat(80))
    println()
    println("# Distant Horizons - Performance Optimized Configuration")
    println("# Generated by DH Diagnostics")
    println()
    ...
    if (maxGenFinding != null) {
        val current = maxGenFinding.currentValue
        val recommended = maxGenFinding.recommendedValue
        println("# maxGenerationRequestDistance controls how far DH generates LODs.")
        println("# Current: $current, Recommended: $recommended")
        println("maxGenerationRequestDistance=$recommended")
        println()
    }
    
    if (maxChunkRadiusFinding != null) {
        val current = maxChunkRadiusFinding.currentValue
        if (current == 0) {
            println("# generationMaxChunkRadius: 0 means unbounded (risky).")
            println("# Recommended: 128 to limit concurrent chunk generation.")
            println("generationMaxChunkRadius=128")
        } else {
            println("# generationMaxChunkRadius: Currently $current (bounded).")
            println("generationMaxChunkRadius=$current")
        }
        println()
    }
    
    println("# Additional recommendations:")
    println("# - Monitor queue sizes with /dhdiag diagnostics")
    println("# - Track queue growth with /dhdiag queue-rate")
    println("# - Check tick history with /dhdiag history")
    println()
    println("-".repeat(80))
    println()
}
