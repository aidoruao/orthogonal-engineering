...
public class ServerPlayerWrapper implements IServerPlayerWrapper {
    ...
    private static final ConcurrentMap<UUID, ServerPlayerWrapper> serverPlayerWrapperMap = new MapMaker().weakKeys()
        .weakValues()
        .makeMap();
    ...
    private ServerPlayerWrapper(EntityPlayerMP serverPlayer) {
    ...
    public IServerLevelWrapper getLevel() {
        WorldServer level = null; // TODO((IMixinServerPlayer)
                                  // this.getServerPlayer()).distantHorizons$getDimensionChangeDestination();
        if (level == null) {
    ...
    @Override
    public int hashCode() {
        return Objects.hashCode(this.serverPlayer.getUniqueID());
    }
