...
public class MinecraftScreen {
    ...
    private static class ConfigScreenRenderer extends DhScreen {
        ...
        public void initGui() {
            ...
            /*
             * this.list = new ConfigListWidget(this.minecraft, this.width, this.height, 0, 0, 25); // Select the area
             * to tint
        ...
        public void drawScreen(int mouseX, int mouseY, float delta) {
            ...
            // this.list.render(matrices, mouseX, mouseY, delta); // Renders the items in the render list (currently
            // only used to tint background darker)

            screen.mouseX = mouseX;
            screen.mouseY = mouseY;
            screen.render(delta); // Render everything on the main screen

            super.drawScreen(mouseX, mouseY, delta); // Render the vanilla stuff (currently only used for the background
                                                     // and tint)
        }
     ...
     * {
     * public ConfigListWidget(Minecraft minecraftClient, int canvasWidth, int canvasHeight, int topMargin, int
     * botMargin, int itemSpacing)
     * {
     * #if MC_VER < MC_1_20_4
