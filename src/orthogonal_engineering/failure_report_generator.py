...
class FailureReportGenerator:
    ...
    def _calculate_methodology_health_score(self) -> MethodologyHealthScore:
        ...
        return MethodologyHealthScore(
            overall_score=overall_score,
            falsifiability_score=falsifiability_score,
            correspondence_score=correspondence_score,
            transparency_score=transparency_score,
            reproducibility_score=reproducibility_score,
            tool_validation_score=tool_validation_score,
            critical_issues=critical_issues,
            improvement_recommendations=recommendations,
        )

    def _calculate_falsifiability_score(self):
        detector = self.failure_statistics.detector_failures
        return max(0.0, 1.0 - (detector * 0.2))

    def _calculate_correspondence_score(self):
        corr = self.failure_statistics.correspondence_failures
        return max(0.0, 1.0 - (corr * 0.2))

    def _calculate_transparency_score(self):
        return 0.8

    def _calculate_reproducibility_score(self):
        repro = self.failure_statistics.reproducibility_failures
        return max(0.0, 1.0 - (repro * 0.2))

    def _calculate_tool_validation_score(self):
        return max(0.0, 1.0 - (self.failure_statistics.critical_failures * 0.3))

    def _generate_markdown_health_report(self, metadata, health_score):
        status = health_score.get_health_status()
        return f"# Methodology Health Report\\n\\nStatus: {status}\\nScore: {health_score.overall_score:.2f}"

    def _generate_json_health_report(self, metadata, health_score):
        import json
        return json.dumps({"status": health_score.get_health_status(), "score": health_score.overall_score}, indent=2)


def generate_failure_report(repository_path, output_dir="failure_reports", format="markdown"):
    """Curated API entry point for failure report generation."""
    fmt = ReportFormat.MARKDOWN if format == "markdown" else ReportFormat.JSON
    generator = FailureReportGenerator(repository_path, output_dir)
    return generator.generate_methodology_health_report(fmt)
