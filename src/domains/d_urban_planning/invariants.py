"""D_URBANPLANNING invariant checks."""

from datetime import datetime
from src.domains.d_urban_planning.implementation import (
    UrbanPlanningRecord,
    UrbanPlanningStatus,
    UrbanPlanningComplianceChecker,
)


def check_compliance_deterministic() -> bool:
    """Invariant: Compliance checks produce consistent results."""
    checker = UrbanPlanningComplianceChecker()
    
    compliant_record = UrbanPlanningRecord(
        record_id="TEST001",
        status=UrbanPlanningStatus.COMPLIANT
    )
    result = checker.check_compliance(compliant_record)
    assert result["compliant"] is True, "Compliant record should pass"
    
    non_compliant = UrbanPlanningRecord(
        record_id="TEST002",
        status=UrbanPlanningStatus.NON_COMPLIANT
    )
