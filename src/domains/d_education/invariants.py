"""D_EDUCATION invariant checks."""

from datetime import datetime
from src.domains.d_education.implementation import (
    EducationRecord,
    EducationStatus,
    EducationComplianceChecker,
)


def check_compliance_deterministic() -> bool:
    """Invariant: Compliance checks produce consistent results."""
    checker = EducationComplianceChecker()
    
    compliant_record = EducationRecord(
        record_id="TEST001",
        status=EducationStatus.COMPLIANT
    )
    result = checker.check_compliance(compliant_record)
    assert result["compliant"] is True, "Compliant record should pass"
    
    non_compliant = EducationRecord(
        record_id="TEST002",
        status=EducationStatus.NON_COMPLIANT
    )
