"""D_POLICEPROCEDURE invariant checks."""

from datetime import datetime
from src.domains.d_police_procedure.implementation import (
    PoliceProcedureRecord,
    PoliceProcedureStatus,
    PoliceProcedureComplianceChecker,
)


def check_compliance_deterministic() -> bool:
    """Invariant: Compliance checks produce consistent results."""
    checker = PoliceProcedureComplianceChecker()
    
    compliant_record = PoliceProcedureRecord(
        record_id="TEST001",
        status=PoliceProcedureStatus.COMPLIANT
    )
    result = checker.check_compliance(compliant_record)
    assert result["compliant"] is True, "Compliant record should pass"
    
    non_compliant = PoliceProcedureRecord(
        record_id="TEST002",
        status=PoliceProcedureStatus.NON_COMPLIANT
    )
