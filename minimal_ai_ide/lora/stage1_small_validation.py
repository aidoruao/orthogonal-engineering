#!/usr/bin/env python3
"""
STAGE 1: SMALL VALIDATION TRAINING
===================================

Implements the Stage 1 training directive:
- CPU fallback (CUDA may be unavailable)
- Minimal batch (20 samples, 1 epoch)
- Freeze all non-attention parameters
...
class TrainingExample:
    ...
    def validate_popperian(self) -> bool:
        ...
        return any(keyword in text for keyword in ['falsifiable', 'falsification', 'testable', 'counterexample'])
            ...
            for i, line in enumerate(f):
                ...
                try:
                    ...
                    example = TrainingExample(
                        ...
                        constraints=data.get('constraints', ['LOGOS', 'CHALCEDON', 'GRACE'])
                    )
...
class Stage1TrainingSystem:
    """Stage 1: Small validation training system"""
    ...
    def _setup_logging(self):
        ...
        log_file = f"{log_dir}/stage1_training_{timestamp}.log"
    ...
    def _validate_governance(self) -> Tuple[bool, List[str]]:
        ...
        if MAX_TRAINING_MINUTES > 60:
            violations.append(f"MAX_TRAINING_MINUTES={MAX_TRAINING_MINUTES} exceeds 60 minute limit")
    ...
    def load_model_and_tokenizer(self):
        ...
        self.peft_config = LoraConfig(
            ...
            lora_alpha=16,
            lora_dropout=0.1,
            target_modules=["c_attn", "c_proj", "c_fc"],  # Attention modules only
            bias="none",
        )
        ...
        # Freeze all non-LoRA parameters (as per directive)
    ...
    def train(self, dataset_path: str, output_dir: str) -> TrainingResult:
        ...
        try:
            ...
            # Training loop
            self.model.train()
            self.logger.info(f"Starting training: {MAX_EPOCHS} epoch(s), {len(dataloader)} batch(es)")
            ...
            for epoch in range(MAX_EPOCHS):
                ...
                for batch_idx, batch in enumerate(dataloader):
                    ...
                    if elapsed_minutes > MAX_TRAINING_MINUTES:
                        self.logger.warning(f"Time limit reached: {elapsed_minutes:.1f} minutes")
                        break
...
def main():
    """Main CLI for Stage 1 training"""
    import argparse

    parser = argparse.ArgumentParser(
        description="Stage 1: Small Validation Training",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
EXAMPLES:
  # Run Stage 1 training with default parameters
  python lora/stage1_small_validation.py

  # Run with custom dataset and output
  python lora/stage1_small_validation.py \\
    --dataset lora_dataset/lora_dataset_augmented.jsonl \\
    --output trained_lora_stage1 \\
    --model distilgpt2

  # Run with CPU explicitly
  python lora/stage1_small_validation.py --device cpu
...
"""
    ...
    print("=" * 70)
    print("STAGE 1: SMALL VALIDATION TRAINING")
    print("=" * 70)
    ...
    trainer = Stage1TrainingSystem(model_name=args.model)
    ...
    if result.success:
        ...
        print(f"   Loss reduction: {result.loss_reduction:.4f} ({result.initial_loss:.4f} → {result.final_loss:.4f})")
        print(f"   NaN events: {result.nan_events}")
        print(f"   Christ score: {result.christ_score:.3f}")
        print(f"   Governance compliant: {'Yes' if result.governance_compliant else 'No'}")
        ...
        try:
            ...
            harness.update_stage(2, "Stage 1 training complete, ready for CUDA training")
    ...
    if result.success and result.christ_score >= 0.7:
        ...
        print("2. Test trained model with: python test_trained_model.py --model trained_lora_stage1")
        print("3. Proceed to Stage 2 (CUDA training) when CUDA is configured")
        print("4. Update test harness registry with new test cases")
    else:
